...
import net.arkx.payementservice.exceptions.orders.OrderIdNullException;
import net.arkx.payementservice.exceptions.orders.OrderNotFoundException;
import net.arkx.payementservice.exceptions.orders.SameOrderIdException;
import net.arkx.payementservice.exceptions.paymentMethod.PaymentMethodNullException;
import net.arkx.payementservice.exceptions.paymentMethod.SamePaymentMethodIdException;
...
public class PaymentService {
    ...
    public Payment findPaymentById(Long id) {
        ...
        if (payment != null) {
            ...
            throw new PaymentNotFoundException("Payment not found for payment with id: " + id);
        }
    }
    ...
    public void addPayment(Payment payment) {
        ...
        if (payment.getPaymentMethod() == null) {
            throw new PaymentMethodNullException("Payment method can't be null");
        }
        if (payment.getOrderId() == null) {
            throw new OrderIdNullException("Order Id can't be Null");
        } else {
    ...
    public void updateAmountPayment(Long idPayment, Long newAmount) {
        ...
        if (payment != null) {
            ...
            throw new PaymentNotFoundException("Payment not found for payment with id: " + idPayment);
        }
    }
    // Update Order Id
    @Transactional
    public void updateOrderId(Long idPayment, Long newOrderId){
        ...
        if(payment!= null){
            if(newOrderId>0&& !payment.getOrderId().equals(newOrderId)){
                ...
                throw new SameOrderIdException("Can't change the order Id by the same value");
            }
        }
        else{
            throw new PaymentNotFoundException("Payment not found for payment with id: "+idPayment);
    ...
    public void updatePaymentMethodId(Long idPayment, PaymentMethod newPaymentMethod) {
        Payment payment = paymentRepository.findById(idPayment)
                .orElseThrow(() -> new PaymentNotFoundException("Payment not found for payment with id: " + idPayment));

        PaymentMethod paymentMethod = paymentMethodRepository.findById(newPaymentMethod.getId())
                .orElseThrow(() -> new PaymentNotFoundException("Can't find the Payment Method Id: " + newPaymentMethod.getId()));
    ...
    public void deletePayment(Long id) {
        Payment paymentToDelete = paymentRepository.findById(id).orElseThrow(() -> new PaymentNotFoundException("Payment not found for payment with id: " + id));
